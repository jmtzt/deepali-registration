r"""Optimization-based pairwise image registration."""

from pathlib import Path
from timeit import default_timer as timer
from typing import Any, Dict, Mapping, Optional, Set, Sequence, Tuple, Union
...
from losses import PairwiseImageRegistrationLoss, weight_channel_names, DMMRLoss
...
def register_pairwise(
    ...
    # Read input images
    start = timer()
    target_keys = set(loss_config.keys()) | set(weight_channel_names(loss_weights).values())
    target_image, target_chns = read_images(target, names=target_keys, device=device)
    source_image, source_chns = read_images(source, names=loss_config.keys(), device=device)
    if verbose > 3:
    ...
    # Low res is 2, mid res is 1, high res is 0
    # target_pyramid = {0: target_pyramid[0]}
    # source_pyramid = {0: source_pyramid[0]}
    ...
    for level in range(coarsest_level, finest_level - 1, -1):
        ...
        # Write initial source image and flow field
        ...
        if outdir and debug > 2:
            engine.register_eval_hook(
                write_result_hook(
                    level=level, grid=target_image.grid(), channels=source_chns, outdir=outdir
                )
            )
        engine.run()
        # Report result
        if verbose > 0 or (outdir and debug > 0):
            ...
            # Resulting transform of current level
            if outdir and debug > 0:
                write_result(
                    result,
                    grid=target_image.grid(),
...
def append_mask(
    ...
    if "img" in channels:
        ...
        mask = U.threshold(data[slice(*channels["img"])], lower_threshold, upper_threshold)
    else:
...
def get_post_transform(
    ...
    center_offset = (
        target_grid.world_to_cube(source_grid.center()).unsqueeze(0) if align_centers else None
    )
    rotation_matrix = (
        source_grid.direction() @ target_grid.direction().t().unsqueeze(0)
        if align_directions
        else None
    )
    transform = None
...
def get_clamp_config(
    ...
    r"""Get thresholds for named image channel.
...
def get_loss_config(config: Dict[str, Any]) -> Tuple[Dict[str, Dict[str, Any]], Dict[str, float]]:
    r"""Instantiate terms of registration loss given configuration object.
    ...
    for name in sections:
        if name in config:
            if cfg is not None:
                raise ValueError("get_loss_config() keys {sections} are mutually exclusive")
            cfg = config[name]
    if not cfg:
        cfg = "SSD"
    ...
    if isinstance(cfg, dict):
        ...
        raise TypeError("get_loss_config() 'config' \"losses\" must be str, tuple, list, or dict")
    ...
    if not isinstance(weights_config, dict):
        raise TypeError("get_loss_config() 'weights' must be scalar, tuple, list, or dict")
...
def get_model_config(config: Dict[str, Any]) -> Tuple[str, Dict[str, Any], Optional[str]]:
    r"""Get configuration of transformation model to use."""
    ...
    if model_init is not None:
        ...
        model_init = (config_path.parent / model_init).as_posix()
    # Transformation keyword arguments
    model_args = dict(cfg.get(model_name, cfg))
    # optim_args.update(cfg)
    return model_name, model_args, model_init


def get_optim_config(config: Dict[str, Any]) -> Tuple[str, Dict[str, Any], Dict[str, Any]]:
    r"""Get configuration of optimizer to use."""
    cfg = config.get("optim", {})
    cfg = dict(name=cfg) if isinstance(cfg, str) else dict(cfg)
    # Name of optimizer
    if "optimizer" in cfg:
        if "name" in cfg:
            raise ValueError("get_optim_config() keys ('name', 'optimizer') are mutually exclusive")
    ...
    # Optimizer keyword arguments (excl. optimizer class name keys starting with uppercase letter)
...
def write_channels(
    ...
    r"""Write image channels."""
    for name, (start, stop) in channels.items():
        image = data[slice(start, stop, 1)]
        if name == "seg":
            image = image.argmax(dim=0, keepdim=True).type(torch.uint8)
        elif name == "msk":
            image = image.mul(255).clamp_(0, 255).type(torch.uint8)
...
def write_result(
    ...
    r"""Write registration result to output directory."""
    # Warped images
    data = result["source"]
    ...
    if data.dtype == torch.bool:
        data = data.type(torch.uint8).mul_(255)
